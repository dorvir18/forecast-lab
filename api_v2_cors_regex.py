...
# api_v2_cors_regex.py — FastAPI backend with robust CORS (regex) + /health
import io, base64, sys
import pandas as pd
from fastapi import FastAPI, UploadFile, File, Form, Request
from fastapi.middleware.cors import CORSMiddleware
from typing import Optional
from arima_core_v2 import read_input_df, run_all_methods_for_group

app = FastAPI(title="ARIMA/SARIMA API v2 — robust CORS + health")

# ✅ Robust CORS: allow demandplanningcourse.com (with/without www) and any *.tilda.ws
app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=r"https://(www\.)?demandplanningcourse\.com$|https://([a-zA-Z0-9-]+\.)?tilda\.ws$",
...
@app.post("/upload")
async def upload(request: Request, file: UploadFile = File(...), sheet: Optional[str] = Form(None)):
    _log(f"[upload] from {request.client.host} file={file.filename}")
...
async def forecast(request: Request,
    ...
    _log(f"[forecast] from {request.client.host} file={file.filename} acc={account} prod={product} H={horizon}")
    content = await file.read()
    if file.filename.lower().endswith((".xlsx",".xls")):
        df = pd.read_excel(io.BytesIO(content), sheet_name=sheet) if sheet else pd.read_excel(io.BytesIO(content))
    elif file.filename.lower().endswith(".csv"):
